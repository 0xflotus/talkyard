...
object ViewPageController {
  ...
  val inlineScriptHashes =
    // The inline script: debiki.internal.renderPageInBrowser();
    "'sha256-DbriJk8xFrYMpSNIQz6xt14pbM1K2PAvQILrWzbM3Aw=' " +
    ""
  ...
  def addVolatileJsonAndPreventClickjacking2(pageHtmlNoVolData: String,
    ...
    val scriptSrcPolicy: St =
          if (!featureFlags.contains("ffUseScriptSrcSelfCsp")) ""
            ...
            s"script-src 'self' $okOrigins $inlineScriptHashes; "
          }
    ...
    // Ok secure policy:  default-src 'none';
    // and then 'self' for all of:
