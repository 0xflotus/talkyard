...
const expectedTopicLinkPreviewTitleHtml =
        '>Title with http://not.title.link.com and ' +
        '&lt;a href="http://not.title.link2.co"&gt;' +
        'http://not.title.link.co&lt;/a&gt; and https://https-link.ex.co</a>';
...
const linkMaybeInCode = '<a href="http://not-link-1.ex.co">ln_txt</a>';
...
describe(`link-previews-http-to-https.1br.test.ts  TyTE2ELNPVHTTPS`, () => {
  ...
  it(`Maria starts composing a new topic`, () => {
    ...
    maria_brA.editor.editTitle("HTTPS and CDN link previews test");
  });


  it(`She adds a link to Michael's topic — but http not https;
  ...
  it(`... the http: text wasn't changed to https:`, () => {
    linkPreviewHtml = maria_brA.waitAndGetVisibleHtml(
          utils.makePreviewOkSelector('InternalLink'));

    assert.includes(linkPreviewHtml, expectedTopicLinkPreviewTitleHtml);
  ...
  it(`Maria submits the new topic`, () => {
    maria_brA.complex.saveTopic({ title: "HTTPS and CDN link previews test" });
  });
  ...
  it(`... and there's the previewed topic's title and body — with any http: inside
        unchanged, since isn't actually links, but plain text`, () => {
  ...
  it(`In Maria's reply, there's a preview of the linked page title and Owen's reply
    ...
    assert.includes(html, expectedTopicLinkPreviewTitleHtml);
  ...
  it(`Maria posts a https link to Owen's reply`, () => {
    maria_brA.complex.replyToOrigPost(owensReplyUrl());
  });


  it(`... the resulting reply html looks the same as with the http: link`, () => {
